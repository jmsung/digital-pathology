...
def train_model(args, configs, preloaded_pdac, preloaded_external, preloaded_ncc):
    ...
    """
    # Unpack internal PDAC data
    PDAC_Features, Coords_PD, Barcodes_PD, Clinical_PD_Time, Clinical_PD_Event = preloaded_pdac
    internal_indices = np.arange(len(PDAC_Features))
    train_idx, valid_idx = train_test_split(internal_indices, test_size=0.2, random_state=42)
    
    # Internal training set
    internal_train_features = [PDAC_Features[i] for i in train_idx]
    internal_train_time = Clinical_PD_Time.iloc[train_idx]
    internal_train_event = Clinical_PD_Event.iloc[train_idx]
    
    # Internal validation set remains unchanged
    VALID_FEATURES = [PDAC_Features[i] for i in valid_idx]
    VALID_TIME = Clinical_PD_Time.iloc[valid_idx]
    VALID_EVENT = Clinical_PD_Event.iloc[valid_idx]
    ...
    for Epoch in range(args.Epoch):
        ...
        for batch_idx in tqdm(range(0, len(train_idx_list), random.randint(8, 32))):
            ...
            for idx in range(batch_idx, batch_idx + random.randint(8, 32)):
