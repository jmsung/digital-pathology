...
class ABMIL(nn.Module):
    def __init__(self, conf):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(self.L, self.ATTENTION_BRANCHES)  # matrix w (or vector w if ATTENTION_BRANCHES==1)
        )
    ...
    def forward(self, x, return_att=False):
        # Expect x to be of shape [1, K, M]; squeeze the first dimension to get [K, M]
        H = x.squeeze(0)
