...
def loss_acmil(pred, y, conf, criterion, device):
        sub_preds, slide_preds, attn = pred
        if conf.n_token > 1:
            loss0 = criterion(sub_preds, y.repeat_interleave(conf.n_token))
        else:
            loss0 = torch.tensor(0.)
        loss1 = criterion(slide_preds, y)
...
def load_data(indices, Filenames, Labels, Path_Feature, Path_Coord):
    Features, Labels_list, Coords, Barcodes = [], [], [], []
    for slide_idx in indices:
        curBarcode = Filenames[slide_idx]
        with open(f'{Path_Feature}/{curBarcode}.pickle', 'rb') as f:
            Feature = pickle.load(f)
        with open(f'{Path_Coord}/{curBarcode}.pickle', 'rb') as f:
            Coord = pickle.load(f)
        Features.append(Feature)
        Coords.append(np.array(Coord))
        Barcodes.append(curBarcode)
        Labels_list.append(Labels[slide_idx])
    return Features, Labels_list, Coords, Barcodes

def evaluate_model(MODEL, dataLoader, criterion, device, ModelName):
    valid_loss, preds, labels = 0.0, [], []
